 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2013-09-18 14:27:58 macan>
 *
...
void lock_debug(int signr)
{
    ...
    if (ent < 5) {
        gingko_debug(lib, "RECEIVE LOCK DEBUG SIGNAL, print the LOCK TABLE.\n");
        lock_table_print();
    } else {
        /* reset sighandler */
...
void lock_table_init(void)
{
    ...
    gingko_debug(lib, "LOCK TABLE INIT ...\n");
...
void __add_tid(struct lock_entry *le)
{
    ...
    if (!t) {
        gingko_err(lib, "xzalloc() struct __tid failed\n");
        return;
    }
    t->tid = pthread_self();
    ...
    gingko_debug(lib, "add %lx le %p to list\n", t->tid, le->p);
...
void __del_tid(struct lock_entry *le)
{
    ...
    list_for_each_entry_safe_reverse(pos, n, &le->tid, list) {
        if (pos->tid == tid) {
            list_del(&pos->list);
            xfree(pos);
            gingko_debug(lib, "del %lx le %p from list\n", tid, le->p);
            xlock_unlock(&le->lock);
            return;
        }
    }
    xlock_unlock(&le->lock);
    gingko_err(lib, "Internal error on deleting TID %lx on le %p.\n", 
               tid, le->p);
    lock_table_print();
    *((int *)0) = 1;
}
...
struct lock_entry *__find_create_lock_entry(void *p)
{
    ...
    if (!le) {
        ...
        if (!le) {
            gingko_err(lib, "xzalloc() lock_entry failed.\n");
            return NULL;
        }
        /* init the lock_entry */
            ...
            if (atomic64_read(&pos->trl) || atomic64_read(&pos->twl) ||
                ...
                if (atomic64_read(&pos->rl)) {
                    ...
                    list_for_each_entry(t, &pos->tid, list) {
                        ...
                        if (bts){
                            for (i = 0; i < t->size; i++) {
                                gingko_info(lib, "[%lx] -> %s\n", t->tid, bts[i]);
                            }
                            free(bts);
                        } else {
                            gingko_info(lib, "[%lx] -> BACKTRACE SYMBOLS ERROR.\n",
                                        t->tid);
                        }
                    }
                    xlock_unlock(&pos->lock);
