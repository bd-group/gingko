 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2013-09-18 14:27:18 macan>
 *
...
void lib_timer_start(struct timeval *begin)
{
    if (gettimeofday(begin, NULL)) {
        gingko_err(lib, "gettimeofday() failed.\n");
    }
}
...
{
    gingko_debug(lib, "%ld %ld -> %ld %ld\n", begin->tv_sec, begin->tv_usec,
                 end->tv_sec, end->tv_usec);
    gingko_info(lib, "ECHO\t %lf us\n", ((end->tv_sec - begin->tv_sec) * 
                                       1000000.0 +
